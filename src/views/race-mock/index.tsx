...
import { MOCK_fetchRaceData } from "../../api/race-data.service";
import {
  Typography,
  Table,
  TableBody,
...
export default function RaceMock() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const response = await MOCK_fetchRaceData();
        setData(response);
      } catch (error: any) {
  ...
  return (
    ...
    >
      <AccordionComponent
        title="Race Mock - ¿Que hace esta vista?"
        description="En esta vista lo que estoy implementando es consumir un mock (copia response del endpoint -> https://pastebin.com/raw/zCrY8y8R ) y ordenar la
            respuesta por posición, tiempo y nombre del piloto o corredor. En este caso el front end tiene toda la responsabilidad y carga, es decir, se encarga de guardar el mock, invocarlo, simular una llamada a un servicio(setTimeout 5000ms), 
            aplicar el filtro, ordenamiento y luego renderizar la informacion en la tabla."
        tecnology="React v-18 + typescript y material-ui 5.15 para crear componentes y views."
      />
